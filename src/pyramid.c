...
int main()
{
    ...
    for (lower48 = 0; ; lower48++) {
        ...
        Pos jp_pos;
        if (!getStructurePos(Jungle_Temple, version, lower48, 0, 0, &jp_pos))
            continue;

        if (abs(bt_pos.x-jp_pos.x) >= max_dist || abs(bt_pos.z-jp_pos.z) >= max_dist)
            continue;
        ...
        for (upper16 = 0; upper16 < 0x10000; upper16++)
        {
            ...
            int cond = tnt_count >= 1 && (iron_count >= 12 || (iron_count >= 10 && gold_count >= 2));

            if (!cond)
                continue;

            Pos spawn_pos = getSpawn(&g);
            if (spawn_pos.x >= 16*2 || spawn_pos.z >= 16*2)
                continue;
            
            if (!isViableStructurePos(Treasure, &g, bt_pos.x, bt_pos.z, 0))
                continue;

            if (!isViableStructurePos(Jungle_Temple, &g, jp_pos.x, jp_pos.z, 0))
                continue;
...
// stdbuf -oL -eL ./build/pyramid > seed/pyramid.txt 2>&1